CREATE OR REPLACE PROCEDURE FINOPS.L3_TEMPLATE.UE_DEPLOYMENT("CUST_NAME" VARCHAR, "DEPLOYMENT_VALUE" VARCHAR)
RETURNS VARCHAR
LANGUAGE PYTHON
RUNTIME_VERSION = '3.11'
PACKAGES = ('snowflake-snowpark-python')
HANDLER = 'ue_deployment'
EXECUTE AS OWNER
AS '
...
def ue_deployment(session, cust_name: str, DEPLOYMENT_VALUE: str):
    try:
        # define variables
    ...
    #Create Queries to be run concurrently
        q1 = session.sql(f"""
        call {schema}.ueq(''{customer_name}'',''{deployment}'')
        """)
        q2 = session.sql(f"""
        call {schema}.uem(''{customer_name}'',''{deployment}'')
        """)
        q3 = session.sql(f"""
        call {schema}.uew(''{customer_name}'',''{deployment}'')
        """)
        dfs = [q1,q2,q3]
