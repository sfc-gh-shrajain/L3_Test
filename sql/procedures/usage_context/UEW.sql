CREATE OR REPLACE PROCEDURE FINOPS.L3_TEMPLATE.UEW("CUST_NAME" VARCHAR, "DEPLOYMENT_NAME" VARCHAR)
RETURNS VARCHAR
LANGUAGE PYTHON
RUNTIME_VERSION = '3.11'
PACKAGES = ('snowflake-snowpark-python')
HANDLER = 'insert_data_into_uew'
EXECUTE AS OWNER
AS '
...
def insert_data_into_uew(session,cust_name: str, deployment_name: str):
    try:
        # Define and set variables
        customer_name = cust_name.replace('' '', ''_'').upper()
        schema = f''FINOPS_OUTPUTS.{customer_name}''
        ue_table = f''{schema}.UEW''
        ...
        # One year prior beginning date (from original UEW)
        prior_one_year_beginning_date = session.sql(f"SELECT dateadd(year, -1, dateadd(month, -3, ''{current_quarter_start_date}''::DATE))").collect()[0][0]
        prior_one_year_beginning_date = prior_one_year_beginning_date.strftime(''%Y-%m-%d'')

        # Set deployment variables
        deployment = deployment_name.lower()
        scoped_table = f''{schema}.SCOPED_ACCOUNTS''
        
        # --- START: NEW DS LOGIC (Warehouse Level) ---
        insert_query = f"""
            ...
            WITH
              warehouse_data AS (
                ...
                WHERE
                  f.DS::DATE BETWEEN ''{prior_one_year_beginning_date}'' AND ''{previous_month_end}''
                  AND WAREHOUSE_ID NOT IN (0, -2)
                  AND f.WAREHOUSE_NAME NOT LIKE ''COMPUTE_SERVICE_WH_%''
                  AND f.deployment = ''{deployment}''
                GROUP BY
                  f.ACCOUNT_ID, f.DEPLOYMENT, f.WAREHOUSE_NAME, PERIOD, "MONTH"
              ),
              user_schema_data AS (
                SELECT
                  f.ACCOUNT_ID,
                ...
                WHERE
                  ...
                  AND f.deployment = ''{deployment}''
                GROUP BY
                  f.ACCOUNT_ID, f.DEPLOYMENT, d.WAREHOUSE_NAME, PERIOD, "MONTH"
              )
            SELECT
              w.PERIOD as PERIOD,
              W.MONTH as MONTH,
            ...
            FROM
              warehouse_data AS w
              LEFT JOIN user_schema_data AS us 
                ON w.account_id = us.account_id
                AND w.deployment = us.DEPLOYMENT
                AND w.PERIOD = us.PERIOD
                AND w.Month = us.month
                AND w.WAREHOUSE_NAME = us.WAREHOUSE_NAME;
            -- --- END: NEW DS LOGIC ---
        """
