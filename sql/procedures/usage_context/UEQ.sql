CREATE OR REPLACE PROCEDURE FINOPS.L3_TEMPLATE.UEQ("CUST_NAME" VARCHAR, "DEPLOYMENT_NAME" VARCHAR)
RETURNS VARCHAR
LANGUAGE PYTHON
RUNTIME_VERSION = '3.11'
PACKAGES = ('snowflake-snowpark-python')
HANDLER = 'insert_data_into_ueq'
EXECUTE AS OWNER
AS '
...
def insert_data_into_ueq(session,cust_name: str, deployment_name: str):
    try:
        # Define and set variables
        customer_name = cust_name.replace('' '', ''_'').upper()
        schema = f''FINOPS_OUTPUTS.{customer_name}''
        ue_table = f''{schema}.UEQ''
        ...
        # Two years prior end quarter date (Original UEQ logic)
        ...
        # Two years prior beginning date (Original UEQ logic)
        ...
        insert_query = f"""
            ...
            WITH
              warehouse_data AS (
                ...
                WHERE
                  f.DS::DATE BETWEEN ''{prior_two_years_beginning_date}'' AND ''{prior_two_years_end_quarter_date}''
