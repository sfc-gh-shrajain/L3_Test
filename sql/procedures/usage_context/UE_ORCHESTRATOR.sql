CREATE OR REPLACE PROCEDURE FINOPS.L3_TEMPLATE.UE_ORCHESTRATOR("CUST_NAME" VARCHAR)
RETURNS VARCHAR
LANGUAGE PYTHON
RUNTIME_VERSION = '3.11'
PACKAGES = ('snowflake-snowpark-python')
HANDLER = 'ue_orchestrator'
EXECUTE AS OWNER
AS '
...
def ue_orchestrator(session, cust_name: str):
    try:
        # define variables
 ...
 #       deploy_list = deploy_list[0]
        ...
        for job_number in range(start_with, len(deploy_list), step):
            ...
            call {schema}.ue_deployment(''{customer_name}'',''{deploy_list[job_number][0]}'')
            """).collect_nowait()
