CREATE OR REPLACE PROCEDURE FINOPS.L3_TEMPLATE.WAREHOUSE_ORCHESTRATOR("CUST_NAME" VARCHAR)
RETURNS VARCHAR
LANGUAGE PYTHON
RUNTIME_VERSION = '3.11'
PACKAGES = ('snowflake-snowpark-python')
HANDLER = 'wh_orchestrator'
EXECUTE AS OWNER
AS '
...
def wh_orchestrator(session, cust_name: str):
    try:
        # define variables
 ...
 #       deploy_list = deploy_list[0]
        ...
        for job_number in range(start_with, len(deploy_list), step):
            ...
            call {schema}.warehouse_deployment(''{customer_name}'',''{deploy_list[job_number][0]}'')
            """).collect_nowait()
...
#    end_time = time.time()     
 #       async_jobs = [df.collect_nowait() for df in dfs]
        ...
        return f''Deployments to run: {res}''
