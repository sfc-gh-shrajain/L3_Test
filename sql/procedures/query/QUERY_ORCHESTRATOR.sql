CREATE OR REPLACE PROCEDURE FINOPS.L3_TEMPLATE.QUERY_ORCHESTRATOR("CUST_NAME" VARCHAR)
RETURNS VARCHAR
LANGUAGE PYTHON
...
def query_orchestrator(session, cust_name: str):
 ...
 #       deploy_list = deploy_list[0]
        ...
        for job_number in range(start_with, len(deploy_list), step):
            ...
            call {schema}.query_deployment(''{customer_name}'',''{deploy_list[job_number][0]}'')
            """).collect_nowait()
